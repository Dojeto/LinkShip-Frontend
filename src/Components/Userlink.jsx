...
const Links = () => {
  ...
  const getUserDetails = async()=>{
    const response = await fetch(`https://linkpeti-backend-production.up.railway.app/manage/getall/${username}`,{
      method:"GET",
      credentials: "include",
  ...
  return (
    ...
    <div className="bg-[#0f172a] h-screen">
        ...
        <div className="pt-8 flex justify-center items-center flex-col gap-y-7">
          {userData && test.userinput.appname.map((ele,index)=>{
            return(
              <Button 
                key={index}
                style={{
                ...
                onClick={()=> window.open(`https://${test.userinput.links[index]}`)}
                variant="contained"
              >
                {ele}
              </Button>
